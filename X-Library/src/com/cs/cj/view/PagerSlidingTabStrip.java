...
//		mDividerWidth = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsDividerWidth, mDividerWidth);
		...
		defaultTabLayoutParams.rightMargin = mDividerWidth ;
		expandedTabLayoutParams = new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1.0f);
		expandedTabLayoutParams.rightMargin = mDividerWidth ;
	...
	private class TabView extends TextView {
		...
		public TabView(Context context) {
            super(context);
            this.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
            this.setTypeface(tabTypeface, tabTypefaceStyle);
			ColorStateList cls = getResources().getColorStateList(tabTextColor);
			this.setTextColor(cls);
			this.setBackgroundResource(tabBackgroundResId);
			...
			if (textAllCaps) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
					...
					this.setText(this.getText().toString().toUpperCase(locale));
				}
			}
        }
...
//		tabsContainer.getChildAt(currentPosition).setSelected(true);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			final boolean isSelected = (i == mSelectedTabPosition);
