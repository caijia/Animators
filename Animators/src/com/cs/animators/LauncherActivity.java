	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mFlower.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				mFlowerBomb.getLocationOnScreen(locationbomb);
				int bombx = locationbomb[0];
				int bomby = locationbomb[1];
				
				
				// 305 * 300
				TranslateAnimation animation = new TranslateAnimation(
						0,
						-(x + mFlower.getWidth() / 2 - (bombx + mFlowerBomb.getWidth() / 2)),
						0,
						-(y + mFlower.getHeight() / 2 - (bomby + mFlowerBomb.getHeight() / 2)));
